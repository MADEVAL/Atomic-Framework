...
<div style="display:flex;min-height:calc(100vh - 120px);align-items:center;justify-content:center;padding:2rem;">
    <div style="width:100%;max-width:420px;padding:2rem;border:1px solid var(--atomic-border, #e0e0e0);border-radius:12px;background:var(--atomic-bg-card, #fff);">
        ...
        <form id="auth-form" method="post" action="/register">
            <input type="hidden" name="_csrf_token" value="<?php echo get_csrf_token(); ?>">
            <input type="hidden" name="atomic_nonce" value="<?php echo create_nonce('register'); ?>">

            <div style="margin-bottom:1rem;">
                <label style="display:block;font-size:0.875rem;font-weight:500;margin-bottom:0.35rem;">Name</label>
            ...
            <div style="margin-bottom:1.25rem;">
                <label style="display:block;font-size:0.875rem;font-weight:500;margin-bottom:0.35rem;">Confirm Password</label>
        ...
        <p style="text-align:center;margin-top:1.25rem;color:var(--atomic-text-secondary, #888);font-size:0.9rem;">
            Already have an account? <a href="/login" style="color:#4f9cf7;text-decoration:none;">Sign in</a>
        </p>
    </div>
</div>
<?php
