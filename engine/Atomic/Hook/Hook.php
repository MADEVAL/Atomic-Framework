...
class Hook {
    ...
    public function remove_action(string $tag, ?callable $function_to_remove = null, int $priority = 10): bool {
        if ($this->has_action($tag)) { $this->event->off($tag); return true; }
        return false;
    }

    public function has_action(string $tag, ?callable $function_to_check = null): bool {
        return $this->event->has($tag);
    }

    public function apply_filters(string $tag, mixed $value, mixed ...$extra): mixed {
        $ctx = ['extra' => $extra]; 
    ...
    public function remove_filter(string $tag, ?callable $function_to_remove = null, int $priority = 10): bool {
        return $this->remove_action($tag, $function_to_remove, $priority);
    }

    public function has_filter(string $tag, ?callable $function_to_check = null): bool {
